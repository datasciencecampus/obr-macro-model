...
def get_list_of_model_series_identifiers() -> list:
    ...
    logger.info(f"There are {len(unique_list)} unique ONS time series codes in the model.")
    return unique_list
...
def get_series_ids_and_datasets(series_identifiers: list)-> pd.DataFrame:
    ...
    for series_id in series_identifiers:
        ...
        temp_df = pd.DataFrame.from_dict({"series_id": series_id, "dataset_id": dataset_id}, orient="index").T
        series_dataset_df = pd.concat([series_dataset_df, temp_df])
    logger.info(f"Dataset ids for {len(series_dataset_df)} time series have been downloaded.")
...
def get_an_ONS_time_series(dataset_id: str, timeseries_id: str) -> pd.DataFrame:
    ...
    url = (api_endpoint + '/'.join([x+'/'+y for x, y in zip(api_params.keys(), api_params.values())][::-1])
                        + '/data')
